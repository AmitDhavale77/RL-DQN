...
class ReplayBuffer():
    ...
    def sample(self, batch_size:int)->list:
        """Get a random sample from the replay buffer (based on batch_size)
...
def greedy_action(dqn:DQN, state:torch.Tensor)->int:
    ...
    """
    return int(torch.argmax(dqn(state))) # It calls the forward propogation and returns 2 Q values, and we are choosing the max of them
...
def epsilon_greedy(epsilon:float, dqn:DQN, state:torch.Tensor)->int:
    ...
    Args:
        ...
        dqn: the DQN that selects the action
        state: state at which the action is chosen (only single state,A)
    ...
    p = float(torch.rand(1)) #generates random float value between (0, 1)
    if p>epsilon:
        return greedy_act
    else:
...
def update_target(target_dqn:DQN, policy_dqn:DQN):
    ...
    target_dqn.load_state_dict(policy_dqn.state_dict()) # To stabilize the training process
